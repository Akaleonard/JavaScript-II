...
runners.forEach(function(names) {
  let runNames = names.first_name + " " + names.last_name;
  fullNames.push(runNames);
})
...
let firstNamesAllCaps = [];
  runners.map((elements) => {
  let allCaps = elements.first_name + " " + elements.last_name;
  firstNamesAllCaps.push(allCaps.toUpperCase());
})
...
runners.filter((element) => {
  if (element.shirt_size === "L") {
    let shirtSize = element.shirt_size;
    runnersLargeSizeShirt.push(shirtSize);
